...
def test_scheduler_build_meta():
    ...
    # Verify that full destination locators are populated correctly
    assert len(req_meta.save_spec.dst_locators) == 2
    assert req_meta.save_spec.dst_locators[0].job_name == scheduler.job_name
    assert req_meta.save_spec.dst_locators[0].job_replica_id == scheduler.job_replica_id
