...
def load_as_qkv_large(
    ...
    # dim_size = num_kq_heads * kq_head_dim * 2 + num_v_heads * v_head_dim
    # before: (seq_tile_size, chunk_size, 1, dim_size)
    # after:
    # 1. (seq_tile_size, num_kq_heads, chunk_size, kq_head_dim) * 2
    # 2. (seq_tile_size, num_v_heads, chunk_size, v_head_dim)
...
def load_as_qkv_compact(
    ...
    """Use 1 load per haed to split qkv along last dim and perform transpose."""

    # dim_size = num_kq_heads * kq_head_dim * 2 + num_v_heads * v_head_dim
    # before: (seq_tile_size, chunk_size, 1, dim_size)
    # after:
    # 1. (seq_tile_size, num_kq_heads, chunk_size, 1, kq_head_dim) * 2
    # 2. (seq_tile_size, num_v_heads, chunk_size, 1, v_head_dim)
...
def load_and_mask_states(
    ...
    """Load correct state from either HBM, prev tile, or zero initialized."""
    ...
    for idx in range(cfgs.seq_tile_size):
        s_idx = metadata_ref.st_idx_to_s_idx[p_id, idx]
        real_size = metadata_ref.st_idx_to_b_size[p_id, idx]
        ...
        if prev_conv_scratch_ref is not None:
            prev_tile_conv = prev_conv_scratch_ref[idx]
        ...
        if prev_recurrent_scratch_ref is not None:
            prev_tile_recurrent_scratch = prev_recurrent_scratch_ref[idx]
