...
from typing import TYPE_CHECKING, Any, Literal, Optional, Set, Tuple, List, Dict
...
class RaidenOffloadConnectorScheduler:
    ...
    def __init__(self, vllm_config: VllmConfig):
        ...
        # Map ReqId -> list(Hash, physical CpuChunkId, RaidenLocator) for active in-flight save tracking
        self._pending_save_hashes: dict[ReqId, list[tuple[int, CpuChunkId, RaidenLocator]]] = defaultdict(list)
    ...
    def get_num_new_matched_tokens(self, request: "Request", num_computed_tokens: int) -> tuple[int, bool]:
        ...
        if num_blocks_to_load > 0:
            num_avail_staging = self.staging_buffer_manager.get_num_free_staging_blocks()
            if num_blocks_to_load > num_avail_staging:
                num_blocks_to_load = num_avail_staging
            ...
            if num_blocks_to_load > 0:
                ...
                # Accurately extract all locator info from RaidenId
                for i in range(load_start_idx, load_start_idx + num_blocks_to_load):
                    _, raiden_ids = matched[i]
                    # Assuming data is in the first matching replica
                    r_id = raiden_ids[0]
    ...
    def _prepare_save_spec(self, tracker: RaidenRequestTracker, is_finished: bool) -> Optional[RaidenSaveSpec]:
        ...
        if should_save:
            ...
            num_avail_staging = self.staging_buffer_manager.get_num_free_staging_blocks()
            if num_blocks_to_save > num_avail_staging:
                num_blocks_to_save = num_avail_staging
    ...
    def build_connector_meta(self, scheduler_output: SchedulerOutput) -> RaidenConnectorMetadata:
        ...
        for req_id, _load_spec in self._pre_load_specs.items():
            self.staging_buffer_manager.free(req_id, "load")
    ...
    def start_load_kv(self, fwd_ctx: "ForwardContext"):
        ...
        for req_meta in meta.requests_meta:
            ...
            if load_spec and load_spec.can_load:
                ...
                # Inspect src_locators to see if we need network transfers or local copies
                remote_src_chunks = []
                local_src_chunks = []
                local_dst_blocks = []
                ...
                if remote_src_chunks:
                    ...
                    # In a real app we might do: self.raiden_manager.h2h_read(peer=..., src_block_ids=...)
    ...
    def wait_for_save(self):
        ...
        for req_meta in meta.requests_meta:
            ...
            if save_spec and not save_spec.skip_save and save_spec.src_blocks:
                ...
                logger.debug(
                    f"Raiden Worker: Async saving {len(src_blocks)} blocks for {req_id} "
                    f"to target locators: {save_spec.dst_locators}"
                )
